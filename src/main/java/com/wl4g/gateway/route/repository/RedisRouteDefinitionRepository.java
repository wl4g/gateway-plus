 ...
 */
package com.wl4g.gateway.route.repository;

import static com.wl4g.gateway.constant.GatewayPlusConstants.CACHE_PREFIX_ROUTES;
...
public class RedisRouteDefinitionRepository extends AbstractRouteRepository {
    ...
    public Mono<Void> save(Mono<RouteDefinition> route) {
        return route.flatMap(routeDefinition -> {
            stringTemplate.opsForHash().put(CACHE_PREFIX_ROUTES, routeDefinition.getId(),
    ...
    public Mono<Void> delete(Mono<String> routeId) {
        return routeId.flatMap(id -> {
            stringTemplate.opsForHash().delete(CACHE_PREFIX_ROUTES, id);
            return Mono.empty();
        });
    }
