...
package com.wl4g.gateway.server;
...
public class SslServerCustomizer implements NettyServerCustomizer {
    ...
    KeyManagerFactory getKeyManagerFactory(Ssl ssl, SslStoreProvider sslStoreProvider) {
        try {
            ...
            KeyManagerFactory keyManagerFactory = (ssl.getKeyAlias() == null)
                    ...
                    : new ConfigurableAliasKeyManagerFactory(ssl.getKeyAlias(), KeyManagerFactory.getDefaultAlgorithm());
    ...
    /**
     * A {@link KeyManagerFactory} that allows a configurable key alias to be
     * used. Due to the fact that the actual calls to retrieve the key by alias
     * are done at request time the approach is to wrap the actual key managers
     * with a {@link ConfigurableAliasKeyManager}. The actual SPI has to be
     * wrapped as well due to the fact that
