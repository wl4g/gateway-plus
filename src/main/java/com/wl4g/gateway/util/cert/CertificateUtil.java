...
package com.wl4g.gateway.util.cert;
...
public abstract class CertificateUtil {
    ...
    private static final int SAN_IPADDRESS = 7;
     ...
     * certificate chain".
     *
     * @param cerChain The certificate chain that will be sorted.
     ...
     * @return Certificate Common Names.
     */
    public static Set<String> getCommonNames(X509Certificate cert) {
        ...
        // Read the SubjectAlternativeNames
        try {
                ...
                for (List<?> list : collection) {
                    if (list.size() == 2) {
                        ...
                        if (key instanceof Integer && value instanceof String) {
                            ...
                            if (intKey == SAN_DNS_NAME || intKey == SAN_IPADDRESS) {
                                names.add(strValue);
                            }
                        }
                    }
    ...
    private static X509Certificate findRootCA(X509Certificate[] cerChain) {
        ...
        for (X509Certificate item : cerChain) {
            ...
            String issuerDN = item.getIssuerX500Principal().getName();
