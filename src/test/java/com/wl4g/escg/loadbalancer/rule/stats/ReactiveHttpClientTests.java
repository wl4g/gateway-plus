...
public class ReactiveHttpClientTests {
    ...
    @Test
    public void testSuccessRepeatedGetRequestNonBlockTimeout() throws Exception {
        long timeoutMs = 5000;
        int count = 5;
        CountDownLatch successLatch = new CountDownLatch(count);
        for (int i = 0; i < count; i++) {
            ...
            HttpClient.create()
                    ...
                    .aggregate()
                    .asString()
                    .timeout(Duration.ofMillis(timeoutMs), Mono.fromRunnable(() -> {
                        System.out.println(currentTimeMillis() + " " + index + " - call timeout");
                    }))
                    .doFinally(signal -> {
                        System.out.println(currentTimeMillis() + " " + index + " - call finally: " + signal);
                        successLatch.countDown();
                    })
                    // main thread non-blocking.
                    .subscribe(response -> {
                        System.out.println(currentTimeMillis() + " " + index + " - call success: " + response.length());
                    }, ex -> {
                        System.out.println(currentTimeMillis() + " " + index + " - call error: " + ex);
                    }, () -> {
                        System.out.println(currentTimeMillis() + " " + index + " - call completion");
                    });
        }

        System.out.println(currentTimeMillis() + " - await...");
        Assertions.assertTrue(successLatch.await(60_000, TimeUnit.MILLISECONDS), () -> "Latch didn't time out");
    }
